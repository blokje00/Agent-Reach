...
class DouyinChannel(Channel):
    name = "douyin"
    description = "Douyin (TikTok China) short videos"
    ...
    def check(self, config=None):
        ...
        if not mcporter:
            return "off", (
                "Requires mcporter + douyin-mcp-server. Installation steps:\n"
        ...
        try:
            ...
            if "douyin" not in r.stdout:
                return "off", (
                    "mcporter is installed but Douyin MCP is not configured. Run:\n"
        ...
        try:
            ...
            return "warn", "MCP is connected but tool list is empty; check if douyin-mcp-server service is running"
        except Exception:
            return "warn", "MCP connection error; check if douyin-mcp-server service is running"
