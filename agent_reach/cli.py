...
def _cmd_install(args):
    ...
    if not dry_run:
        ...
        # Star reminder
        print()
        print("If Agent Reach has been useful to you, give it a Star so more people can find it:")
        print("   https://github.com/Panniantong/Agent-Reach")
        print("   Only takes a second, and it means a lot to an indie developer. Thank you!")
    else:
        print()
...
def _configure_xhs_cookies(value):
    ...
    if mcporter:
        ...
        try:
            ...
            # Note: "已登录" is Chinese for "logged in" — the XiaoHongShu MCP service
            # returns Chinese responses so we check for both Chinese and English.
            if "已登录" in result.stdout or "logged" in result.stdout.lower() or "logged in" in result.stdout.lower():
...
def _cmd_setup():
    ...
    if not shutil.which("mcporter"):
        ...
        try:
            ...
            print("  [!] Unable to check Exa configuration. Please run manually:")
            print("     mcporter config add exa https://mcp.exa.ai/mcp")
        print()
    ...
    if current:
        ...
        if key:
            ...
            print("  ✅ GitHub API rate limit raised to 5000 requests/hour!")
        else:
            print("  Skipped. Public API still works")
    print()

    # Step 3: Reddit proxy
    print("[Optional] Reddit Proxy — full read access for Reddit posts + comments")
    print("  Reddit blocks many IPs; an ISP proxy is needed for direct access")
    print("  Format: http://username:password@IP:port")
    current = config.get("reddit_proxy")
    if current:
        print(f"  Current status: ✅ Configured")
    else:
        proxy = input("  REDDIT_PROXY (press Enter to skip): ").strip()
        if proxy:
            config.set("reddit_proxy", proxy)
            print("  ✅ Reddit full reading enabled!")
        else:
            print("  Skipped. Reddit content is still accessible via search")
    print()

    # Step 4: Groq (Whisper)
    print("[Optional] Groq API — speech-to-text when videos have no subtitles")
    print("  Free tier, register at: https://console.groq.com")
...
def _cmd_check_update():
    ...
    print(f"[!] Unable to check for updates (GitHub returned {resp2.status_code})")
    return "error"
...
def _cmd_watch():
    ...
    print(f"Agent Reach Monitor Report")
    print(f"=" * 40)
